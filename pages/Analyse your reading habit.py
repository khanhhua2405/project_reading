...
lottie_book = load_lottieurl('https://assets2.lottiefiles.com/packages/lf20_acryqbdv.json')
...
row0_2.subheader( 'A Streamlit web app by Khanh Hua')
...
with row1_1:
    ...
    st.markdown(
        "**To begin, please enter the link to your [Goodreads profile](https://www.goodreads.com/) 👇")
...
with row2_1:
    ...
    default_username="139340318-khanh-hua"
...
#per_page: số lượng sách lấy data trong 1 page
#shelf: kệ trong goodreads
#take api from the books you read
@st.cache
...
#take api from the books you plan to read
@st.cache
def get_user_data(user_id, key='ZRnySx6awjQuExO9tKEJXw', v='2', shelf='to-read', per_page='200'):
    api_url_base = 'https://www.goodreads.com/review/list/'
    final_url = api_url_base + user_id + '.xml?key=' + key + \
        '&v=' + v + '&shelf=' + shelf + '&per_page=' + per_page
    contents = urllib.request.urlopen(final_url).read()
...
#sheft = to-read
contents_to_read = get_user_data(user_id=user_id, v='2', shelf='to-read', per_page='200')
contents_to_read = xmltodict.parse(contents_to_read)
...
#shelf = to-read
df_to_read = json_normalize(contents_to_read['GoodreadsResponse']['reviews']['review'])
u_books_to_read = len(df_to_read['book.id.#text'].unique())
u_authors_to_read = len(df_to_read['book.authors.author.id'].unique())
df_to_read['read_at_year'] = [i[-4:] if i != None else i for i in df_to_read['read_at']]
has_records_to_read = any(df_to_read['read_at_year'])

#take the year
df_year = max(df['read_at_year'].dropna())

st.write('')

row_space,col1, col2, col3, row_space1 = st.columns((5))
row_space.write('')
col1.metric("Total books you have read", "{}".format(u_books))
col2.metric("Total books you plan to read", "{}".format(u_books_to_read))
col3.metric("In {}, you have read",'kf')
row_space1.write('')
...
with row3_1, _lock:
    ...
    if has_records:
        ...
        options = st.multiselect('Choose year',year_df['index'], year_df['index'])
        if options:
            year_df = year_df[year_df['index'].isin(options)]
        fig = Figure()
        ax = fig.subplots()
...
with row3_2, _lock:
    ...
    fig = Figure()
    ax = fig.subplots()
    sns.histplot(pd.to_numeric(df['book.publication_year'], errors='coerce').dropna().astype(np.int64), kde_kws={'clip': (0.0, 2020)}, ax=ax, kde=True)
